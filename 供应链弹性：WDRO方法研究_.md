...
* **S2：弹性能力供应商（Elastic Capacity Suppliers），视为“实物期权”**：这些供应商提供灵活的、按需的能力，可以根据实时需求进行调整 18。这种弹性能力被概念化为一种\*\*“实物期权”\*\* ，允许制造商投资于使用能力的“权利”，而无需预先承诺其全部利用 23。实物期权估值（Real Options Valuation, ROV）是一种先进的分析方法，用于在不确定条件下进行决策，评估延迟决策或改变策略的灵活性价值 23。

灵活调整生产资源对于确保稳健性至关重要。在按订单生产环境中管理弹性能力 19以及制造灵活性对供应链绩效的影响 19都是公认的研究领域。双重采购是创建和利用弹性能力的关键机制 21。
