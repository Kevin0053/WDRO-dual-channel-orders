...
现在希望将以上的两阶段Wasserstein鲁棒优化问题利用分段仿射函数（piecewise affine function）进行进一步简化：

$$
\min_{\boldsymbol{x}, \tau} \quad 
c_d \bar{x}_d + c_e x_e + 
\underbrace{
\max_{\mathbb{P} \in \mathbb{B}_\varepsilon(\widehat{\mathbb{P}}_N)} \mathbb{E}_{\mathbb{P}} \left[ -Q(\boldsymbol{x}, \boldsymbol{\xi}) \right]
}_{f_1} + 
\underbrace{
\rho \cdot \max_{\mathbb{P} \in \mathbb{B}_\varepsilon(\widehat{\mathbb{P}}_N)} 
\mathbb{E}_{\mathbb{P}} \left[
\tau + \frac{1}{\mu} \cdot \max \left\{ 0, D_1 + D_2 - x_{tot} - \tau \right\}
\right]
}_{f_2}
$$

其中$f_1,f_2$的定义为如下仿射损失函数：

* $f_1(\boldsymbol{\xi}, \boldsymbol{x}) = -p_1 D_1 - p_2 D_2 + \sum c_d^i x_d^i + \sum c_e^i x_e^i + h \cdot \max\{0, \bar{x}_d - x_d^1 - x_d^2\}+ t \cdot \max\left\{ 0, D_1 + D_2 - \bar{x}_d - x_e \right\}$
* $f_2(\boldsymbol{\xi}, \boldsymbol{x}, \tau) = \rho\left(\tau + \frac{1}{\mu} \cdot \max\{0, D_1 + D_2 - x_{tot} - \tau\}\right)$

现在为了简化表达，将$f_1,f_2$重构整理为如下 **仿射段组合形式**：

$$
\ell(\boldsymbol{\xi}, \boldsymbol{x}, \tau) := \max_k \left( a_k^\top \boldsymbol{\xi} + b_k^\top \boldsymbol{x} + c_k \tau \right)
$$

利用定义好的向量：

* $\boldsymbol{\xi} = (D_1, D_2)^\top$
* $\boldsymbol{x} = (\bar{x}_d,x_e,x_d^1, x_d^2, x_e^1, x_e^2)^\top$

分别解释每一段仿射函数：
**对 $f_1(\boldsymbol{\xi}, \boldsymbol{x})$** 拆解为2段：

* **段 1（无过剩产能时）**：

  $$
  f_{1,1} = -p_1 D_1 - p_2 D_2 + \sum c_d^i x_d^i + \sum c_e^i x_e^i
  $$

  对应：

  * $a_1 = (-p_1, -p_2)$
  * $b_1 = (c_d^1, c_d^2, c_e^1, c_e^2, 0)$
  * $c_1 = 0$

* **段 2（有过剩产能时）**：

  $$
  f_{1,2} = f_{1,1} + h (\bar{x}_d - x_d^1 - x_d^2)
  $$

  对应：

  * $a_2 = a_1 = (-p_1, -p_2)$
  * $b_2 = (c_d^1 - h, c_d^2 - h, c_e^1, c_e^2, h)$
  * $c_2 = 0$


**对 $f_2(\boldsymbol{\xi}, \boldsymbol{x}, \tau)$** 也拆为2段：

* **段 3（需求未满足的惩罚项非零）**：

  $$
  f_{1,3} = t \cdot \max\left\{ 0, D_1 + D_2 - \bar{x}_d - x_e \right\} = t \cdot \left( D_1 + D_2 - \bar{x}_d - x_e \right)
  $$
 对应：
...
* **段 5（惩罚项非零）**：
  ...
  $$
  f_{2,2} = \rho \left(\tau + \frac{1}{\mu}(D_1 + D_2 - x_{\text{tot}} - \tau)\right) = \left(1 - \frac{1}{\mu} \right)\rho\tau + \frac{\rho}{\mu}(D_1 + D_2 - x_{\text{tot}})
  $$

  对应：

  * $a_5 = \left(\frac{\rho}{\mu}, \frac{\rho}{\mu} \right)$
  * $b_5 = \left(-\frac{\rho}{\mu}, -\frac{\rho}{\mu}, -\frac{\rho}{\mu}, -\frac{\rho}{\mu}, 0 \right)$
  * $c_5 = (1 - \frac{1}{\mu})\rho$
...
| $k$ | 含义           | $a_k$                                               | $b_k$                                                                                          | $c_k$                                  |
| --- | ------------ | --------------------------------------------------- | ---------------------------------------------------------------------------------------------- | -------------------------------------- |
| 1   | 主利润项（无剩余产能）  | $(-p_1, -p_2)$                                      | $(c_d^1, c_d^2, c_e^1, c_e^2, 0)$                                                              | 0                                      |
| 2   | 主利润项（有剩余产能）  | $(-p_1, -p_2)$                                      | $(c_d^1-h, c_d^2-h, c_e^1, c_e^2, h)$                                                          | 0                                      |
| 3     | 未满足需求惩罚项 | $(t, t)$ | $(-t, 0, 0, 0, 0, 0)$ | 0     |
| 4   | SLA满足（无违约惩罚） | $(0, 0)$                                            | $(0, 0, 0, 0, 0)$                                                                              | $\rho$                                 |
| 5   | SLA违约（有尾部惩罚） | $\left( \frac{\rho}{\mu}, \frac{\rho}{\mu} \right)$ | $\left( -\frac{\rho}{\mu}, -\frac{\rho}{\mu}, -\frac{\rho}{\mu}, -\frac{\rho}{\mu}, 0 \right)$ | $\rho \left(1 - \frac{1}{\mu} \right)$ |


$$
\ell(\boldsymbol{\xi}, \boldsymbol{x}, \tau) = \max_{k = 1,\dots,5} \left( a_k^\top \boldsymbol{\xi} + b_k^\top \boldsymbol{x} + c_k \tau \right)
$$
